import { menuProducts } from "./menu-tabs.js";
/**
...
async function loadProductSections() {
    try {
        ...
        if (!res.ok)
            throw new Error("Failed to load menu-products.html");
        const html = await res.text();
        ...
        if (!modalRes.ok)
            throw new Error("Failed to load product-modal.html");
        const modalHtml = await modalRes.text();
        const modalContainer = document.querySelector("#product-modal-placeholder");
        if (modalContainer)
