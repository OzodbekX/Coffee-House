...
export const PaymentModal: React.FC<PaymentModalProps> = ({
    ...
    const validateExpiry = (v: string): string | null => {
        ...
        if (expiryDate < new Date(now.getFullYear(), now.getMonth())) return "Card is expired.";
        return null;
    };
    ...
    const handlePayment = async () => {
        ...
        if (nameErr || cardErr || expiryErr || cvvErr) {
            ...
            setMessage({ text: "Please fix the errors before proceeding.", type: "error" });
            return;
        }
        ...
        <div className="payment-modal-overlay">
            <div className="payment-modal">
                ...
                <div className="form-field">
                    <label htmlFor="card-name" className="text-medium">Cardholder Name</label>
                    <input
                        id="card-name"
                        type="text"
                        value={name}
                    ...
                    />
                    {errors.name && (
                        <div className="text-caption" style={{ color: "#b00020" }}>❗ {errors.name}</div>
                    )}
                </div>

                {/* Card Number */}
                <div className="form-field">
                    <label htmlFor="card-number" className="text-medium">Card Number</label>
                    <input
                        id="card-number"
                        type="text"
                        maxLength={19}
                        ...
                        style={{ border: errors.cardNumber ? "1px solid #b00020" : undefined }}
                    />
                    {errors.cardNumber && (
                        <div className="text-caption" style={{ color: "#b00020" }}>❗ {errors.cardNumber}</div>
                    )}
                </div>
                ...
                <div className="payment-row">
                    <div className="form-field">
                        <label htmlFor="expiry" className="text-medium">Expiry (MM/YY)</label>
                        <input
                            id="expiry"
                            type="text"
                            value={expiry}
                            ...
                            style={{ border: errors.expiry ? "1px solid #b00020" : undefined }}
                        />
                        {errors.expiry && (
                            <div className="text-caption" style={{ color: "#b00020" }}>❗ {errors.expiry}</div>
                        )}
                    </div>

                    <div className="form-field">
                        <label htmlFor="cvv" className="text-medium">CVV</label>
                        <input
                            id="cvv"
                            type="text"
                            value={cvv}
                        ...
                        />
                        {errors.cvv && (
                            <div className="text-caption" style={{ color: "#b00020" }}>❗ {errors.cvv}</div>
                        )}
                    </div>
                </div>
